...
- **Networking** — network-engineer prep scoped to OSI layers 1–3, written to be learned from rather than skimmed: each of the 33 cards opens with a plain-English framing, teaches the problem and the mechanism before listing any facts, and expands every acronym (with a glossary card decoding all 80 of them). Covers the Ethernet frame, IPv4 header, switch learning, STP and the other L2 control protocols, router forwarding, ICMP, routing protocols, subnetting and troubleshooting — plus 35 Q&A drills — each with a short answer to deliver, the reasoning behind it, and the likely follow-ups — including a fully worked subnetting practice set, and a 58-card recall drill where each answer is covered until clicked and unfolds with a one-line reason.
