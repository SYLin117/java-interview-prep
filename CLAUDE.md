...
- `leetcode.js` — `const leetcode = [...]` with the 200 popular LeetCode problems (title, difficulty, bucket, category, URL, approach hint, complexity, Java solution). Entries are pre-sorted by `bucket` (16 buckets in display order); `num` is a stable internal key (1..200, NOT in array order) that keys `user-overrides.js`, `descriptions.js` and `algorithms.js`. Existing Garmin/Temu tags live on individual entries; a dated public reported-question snapshot after the array adds CATL, CoStar, Walmart, Amazon, Home Depot, Lowe's, and DoorDash tags by stable `num`. Company tags render as colored pills and filter options.
