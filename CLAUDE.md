...
There are six top-level views, toggled by buttons in the header: **Topics** (the interview question bank — `<div id="topics-view">`), **LeetCode** (`<div id="leetcode-view">`), **Garmin Behavioral** (`<div id="behavioral-view">`), **Supermicro** (`<div id="supermicro-view">` — technical System-Engineer prep sections rendered from `supermicro.js`, same expandable `.q-row` pattern as Topics with a per-topic dropdown filter), **System Design** (`<div id="system-design-view">`), and **MSI** (`<div id="msi-view">`). The toggle uses the `[hidden]` attribute so only one view is in the layout at a time; `setView()` is data-driven via the `VIEW_EL` / `VIEW_HASH` / `VIEW_PLACEHOLDER` maps. URL hash routing: `#leetcode` (or `#lc`) opens the LeetCode view, `#behavioral` the Behavioral view, `#supermicro` (or `#smci`) the Supermicro view, `#systemdesign` (or `#sd`) the System Design view, `#msi` the MSI view; clicking a topic-anchor link (`#core-java`, etc.) auto-switches back to the topics view. The search box and its placeholder swap behavior based on the active view. The Behavioral view renders one `.behavioral-section` per chain (main question as the section title, suggested answer + follow-ups as expandable `.q-row`s) and has its own category dropdown. The System Design view has a **sub-tab toggle** (`.sd-subtab`, state `sdSubtab`): *Concepts* renders the Q&A themes as topic sections of expandable `.q-row`s with a theme dropdown (shown only in this mode); *Case Studies* renders `.sd-case` cards that expand from their header into labelled `.sd-block`s. Search (`applySystemDesignFilters`) targets whichever sub-tab is active. The MSI view renders the `msiProcess` overview card first, then one `.msi-section` per prep area with expandable `.q-row`s sorted easy → hard; search (`applyMsiFilters`) filters rows and hides the process card while a term is active.
