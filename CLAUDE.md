...
- `index.html` — markup, CSS, render logic, the chat widget, and the view toggle (Topics ↔ LeetCode ↔ Companies ↔ System Design ↔ Networking; the Companies view holds the per-company sub-tabs).
...
- `networking.js` — three arrays for the **Networking** view (final-round network-engineer prep, scoped by the hiring manager to OSI layers 1–3): `const networkingGuide = [...]` (33 cards, each `{ id, icon, layer, title, tagline, explain, tags[], blocks: [{ t, h }] }` — `layer` is what the guide's filter dropdown groups by, and its options are built from whatever layers appear in the data), `const networkingQuestions = [...]` (3 sets × `{ id, title, desc, questions: [{ d, q, a }] }` — 35 model answers, including a 10-problem subnetting practice set), and `const networkingDrill = [...]` (5 groups × `{ id, title, items: [{ p, a, why }] }` — 58 recall flashcards; `why` is a one-line anchor revealed with the answer, so missing a card teaches something instead of just restating the fact). Ported from a personal study guide. Every `h`/`a` field is HTML (innerHTML): angle brackets must be `&lt;`/`&gt;` **including inside the ASCII diagrams** in `<pre class="net-pre">` blocks (the Ethernet frame and IPv4 header drawings both contain them). Tables use `<table class="net-table">` and work in guide blocks and Q&A answers alike.
  - **The reader has no networking background** — they're a Java developer prepping for one interview, and the tab is meant to be *studied from*, not used as a cheat sheet. Three conventions follow from that:
    1. Every card carries an `explain` field (HTML, rendered as the "In plain English" box leading the card body) giving a beginner framing that doubles as something they could say out loud.
    2. **Every card teaches before it references.** The first block or two is narrative — usually titled "Start here …" — covering *what problem this solves* and *how the mechanism works*, with a worked example where one helps. Tables, field lists and command lists come after. If you add a topic, add the teaching blocks too; a bare fact table is not usable by this reader.
    3. **Every abbreviation gets expanded on first use**, either inline (`<em>Link Layer Discovery Protocol</em>`) or, inside a table's first column, via `<span class="net-expand">`. Card `net-glossary` is the backstop — an 80-row decode table for every acronym on the page. Add new acronyms there too.
  - Q&A answers follow a fixed three-part shape so they can be studied rather than recited: a `.net-explain` box labelled **"Say this"** with the short interview-length answer, then an `.ex-label` section giving the reasoning behind it, then **"If they push further"** — the likely follow-ups with one-line answers. The subnetting set uses **"Answer"** plus a numbered **"Working"** instead, showing the same four counting steps every time. The two format questions (Ethernet frame, IPv4 header) additionally carry the full diagram and field table inline, because those are the two topics the hiring manager named first and the drill has to stand alone.
  - **Never end a line with a backslash inside a template literal** — it is a line continuation and silently swallows the newline, which collapsed an ASCII topology diagram into gibberish once. Draw box-and-line diagrams with `+`, `-` and `|` only. `grep -n '\\$' networking.js` should return nothing.
  - Watch for **duplication** when adding narrative to a card that already had reference blocks: the teaching block usually supersedes the old prose, and the same explanation twice in one card is worse than either alone. Delete the superseded block rather than leaving both.
...
All these data files are classic (non-module) scripts loaded via `<script src="...">` before the inline script; the inline script then reads `topics` / `extras` / `leetcode` / `behavioral` / `supermicro` / `systemDesignThemes` / `systemDesignCases` / `msiProcess` / `msiSections` / `networkingGuide` / `networkingQuestions` / `networkingDrill` from the shared script realm. Don't change any to `type="module"` — would break the cross-script reference.

## Page views

There are five top-level views, toggled by buttons in the header: **Topics** (the interview question bank — `<div id="topics-view">`), **LeetCode** (`<div id="leetcode-view">`), **Companies** (`<div id="companies-view">`), **System Design** (`<div id="system-design-view">`), and **Networking** (`<div id="networking-view">`). The toggle uses the `[hidden]` attribute so only one view is in the layout at a time; `setView()` is data-driven via the `VIEW_EL` / `VIEW_HASH` / `VIEW_PLACEHOLDER` maps.
...
URL hash routing: `#leetcode` (or `#lc`) opens the LeetCode view, `#companies` the Companies view, `#systemdesign` (or `#sd`) the System Design view, `#networking` (or `#net`) the Networking view; the legacy hashes `#behavioral`, `#supermicro` (or `#smci`), and `#msi` open the Companies view with the matching sub-tab, and selecting a sub-tab rewrites the hash to that company's legacy link so it stays shareable. Clicking a topic-anchor link (`#core-java`, etc.) auto-switches back to the topics view. The search box and its placeholder swap behavior based on the active view; inside Companies it dispatches to the active sub-tab's filter (`applyBehavioralFilters` / `applySupermicroFilters` / `applyMsiFilters`), and switching sub-tabs re-applies the current term. The System Design view has a **sub-tab toggle** (`.sd-subtab`, state `sdSubtab`): *Concepts* renders the Q&A themes as topic sections of expandable `.q-row`s; *Case Studies* renders `.sd-case` cards that expand from their header into labelled `.sd-block`s. Each sub-tab has its own dropdown — `#sd-topic-row` (theme) for Concepts, `#sd-company-row` (company tag) for Case Studies — both rendered once and swapped via the `hidden` attribute. Because `.lc-filter-row` sets an explicit `display: flex`, the `.lc-filter-row[hidden] { display: none }` rule is what actually performs that swap; without it the attribute is inert. Same trap applies to filtering itself: `applySystemDesignFilters` toggles a `.hidden` class on theme sections and case cards, which needs the `.sd-concept-section.hidden` / `.sd-case.hidden` rules to have any effect. Search (`applySystemDesignFilters`) targets whichever sub-tab is active.

The **Networking** view follows the same sub-tab pattern (`.net-subtab`, state `netSubtab`, filter `applyNetworkingFilters`): *Study Guide* renders `networkingGuide` as `.net-card`s that expand from their header into a `.net-explain` box ("In plain English") followed by labelled `.sd-block`s, *Q&A Drills* renders `networkingQuestions` as `.net-qa-section`s of expandable `.q-row`s sorted easy → hard, and *Recall Cards* renders `networkingDrill` as `.net-drill-row`s whose answer column is transparent until the row is clicked (the text stays in the DOM so search still finds it). Sub-tab dropdowns: `#net-layer-row` (layer) for the guide, `#net-section-row` (question set) for the drills — same `hidden`-attribute swap and the same `.lc-filter-row[hidden]` dependency as System Design. Two classes deliberately do NOT get reused even though they share every CSS rule: `.net-subtab` (the SD handler binds to every `.sd-subtab` on the page) and `.net-card` (`applySystemDesignFilters` queries `.sd-case` document-wide and would otherwise hide networking cards). The `.sd-block*` internals *are* reused — no script queries them. New CSS: `.net-table` (bordered tables, scroll inside their own box so a wide table never pushes the page sideways), `.net-pre` (`white-space: pre` — ASCII frame/header diagrams must not wrap), `.net-explain` / `.net-expand` (the plain-English box and the acronym expansions under a table's first column), and the drill rows.
...
The config knows about our cross-script globals (`topics`, `extras`, `leetcode`, `leetcodeUserOverrides`, `leetcodeDescriptions`, `namedAlgorithms`, `problemAlgorithms`, `behavioral`, `supermicro`, `systemDesignThemes`, `systemDesignCases`, `msiProcess`, `msiSections`, `networkingGuide`, `networkingQuestions`, `networkingDrill`) so `const X = ...` in a data file doesn't trigger `no-unused-vars`. Add new data-file globals to `dataFileGlobalsPattern` in `eslint.config.mjs`.
