...
There are three top-level views, toggled by buttons in the header: **Topics** (the interview question bank — `<div id="topics-view">`), **LeetCode** (`<div id="leetcode-view">`), and **Garmin Behavioral** (`<div id="behavioral-view">`). The toggle uses the `[hidden]` attribute so only one view is in the layout at a time; `setView()` is data-driven via the `VIEW_EL` / `VIEW_HASH` / `VIEW_PLACEHOLDER` maps. URL hash routing: `#leetcode` (or `#lc`) opens the LeetCode view, `#behavioral` opens the Behavioral view; clicking a topic-anchor link (`#core-java`, etc.) auto-switches back to the topics view. The search box and its placeholder swap behavior based on the active view. The Behavioral view renders one `.behavioral-section` per chain (main question as the section title, suggested answer + follow-ups as expandable `.q-row`s) and has its own category-chip filter.
