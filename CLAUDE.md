...
- `networking.js` — three arrays for the **Networking** view (final-round network-engineer prep, scoped by the hiring manager to OSI layers 1–3): `const networkingGuide = [...]` (33 reference cards, each `{ id, icon, layer, title, tagline, explain, tags[], blocks: [{ t, h }] }` — `layer` is what the guide's filter dropdown groups by, and its options are built from whatever layers appear in the data), `const networkingQuestions = [...]` (3 sets × `{ id, title, desc, questions: [{ d, q, a }] }` — 37 model answers, including a 10-problem subnetting practice set), and `const networkingDrill = [...]` (5 groups × `{ id, title, items: [{ p, a }] }` — 58 recall flashcards). Ported from a personal study guide. Every `h`/`a` field is HTML (innerHTML): angle brackets must be `&lt;`/`&gt;` **including inside the ASCII diagrams** in `<pre class="net-pre">` blocks (the Ethernet frame and IPv4 header drawings both contain them). Tables use `<table class="net-table">` and work in guide blocks and Q&A answers alike.
  - **The reader has no networking background** — they're a Java developer prepping for one interview. That drives two conventions worth keeping: every card carries an `explain` field (HTML, rendered as the "In plain English" box that leads the card body) giving a beginner-level framing that doubles as something they could say out loud; and **every abbreviation gets expanded on first use**, either inline (`<em>Link Layer Discovery Protocol</em>`) or, inside a table's first column, via `<span class="net-expand">`. Card `net-glossary` is the backstop — an 80-row decode table for every acronym on the page. Add new acronyms there too.
